...
final class SetTest extends BaseBagSuite
{
    ...
    public function test_value_set_to_bag_via_offset_set(): void
    {
        ...
        $bag = $this->utility([]);
        $bag->offsetSet(0, 'foo');
        $this->assertSame([0 => 'foo'], $bag->value());

        // check value is not overwritten
        $bag = $this->utility([0 => 'foo']);
        $bag->offsetSet(0, 'bar');
        $this->assertSame([0 => 'bar'], $bag->value());
        ...
        $bag->offsetSet(0, 'foo');
        $this->assertSame(['foo' => 'bar', 0 => 'foo'], $bag->value());
    ...
    public function test_value_set_to_bag_via_set(): void
    {
        ...
        $this->assertSame([0 => 'foo'], $bag);

        // check value is not overwritten
        $bag = $this->utility([0 => 'foo'])->set(0, 'bar')->value();
        $this->assertSame([0 => 'bar'], $bag);
        ...
        $this->assertSame(['foo' => 'bar', 0 => 'foo'], $bag);

        // check can use key index
        $bag = $this->utility([0 => 'foo'])->set('foo', 'bar')->value();
        $this->assertSame([0 => 'foo', 'foo' => 'bar'], $bag);
